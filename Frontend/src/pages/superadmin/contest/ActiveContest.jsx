...
function ActiveContest() {
  ...
  const fetchAllContests = async () => {
    ...
    try {
      ...
      const activeContests = allData.filter((c) => c.activestatus === true);
  ...
  const handleStatusChange = async (contest) => {
    ...
    if (res?.status) {
      ...
      setContests((prev) =>
        prev.filter((c) => c._id !== contest._id)
      );
    } else {
  ...
  const columns = [
    ...
    {
      name: "Status",
      cell: () => <span className="text-green-600 font-semibold">Active</span>,
    },
    {
      name: "Actions",
      cell: (row) => (
