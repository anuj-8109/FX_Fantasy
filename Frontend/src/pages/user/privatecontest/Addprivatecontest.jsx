...
const AddContest = () => {
  ...
  const contestWrapper = location.state?.contestWrapper;
  ...
  const initialForm = {
    ...
    is_guaranteed: false,
    client_id: client_id || "",
    tournament_id: contestWrapper?.tournament_id || "",
    contest_code: "",
  };

  const [formData, setFormData] = useState(initialForm);

  // Prize distribution dynamic state
  ...
  const addPrizeRow = () => {
    setPrizeDist((prev) => [...prev, { rank: prev.length + 1, amount: "" }]);
  };

  const removePrizeRow = (index) => {
    const updated = prizeDist.filter((_, i) => i !== index);
    setPrizeDist(updated);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    for (let pd of prizeDist) {
      if (!pd.rank || !pd.amount) {
        toast.error("All prize distribution fields must be filled!");
        return;
      }
      if (isNaN(pd.amount)) {
        toast.error("Prize amount must be a number!");
        return;
      }
    }
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white shadow rounded">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input
            type="number"
            name="max_entry_per_user"
            value={formData.max_entry_per_user}
            onChange={handleChange}
            placeholder="Max Entry per User"
            className="w-full p-3 border rounded"
          />
          <input
            type="number"
        ...
        <button
          type="submit"
          className="w-full p-3 bg-blue-600 hover:bg-blue-700 text-white rounded font-semibold transition"
        >
          Add Contest
        </button>
      </form>
