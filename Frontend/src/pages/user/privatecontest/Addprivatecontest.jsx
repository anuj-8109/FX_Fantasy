...
const AddContest = () => {
  ...
  const initialForm = {
    ...
    name: "",
    description: "",
    contest_type: "Mega", // Default Mega
    entry_fee: "",
    total_spots: "",
  ...
  const addPrizeRow = () => {
    const lastTo = prizeDist.length ? parseInt(prizeDist[prizeDist.length - 1].to || 1) : 1;
  ...
  const validateForm = () => {
    ...
    for (let pd of prizeDist) {
      if (!pd.from || !pd.to || !pd.amount || isNaN(pd.amount)) return "All prize rows must be valid numbers";
    }
    return null;
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 bg-white shadow rounded">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Start & End Date */}
        {/* <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input type="datetime-local" name="start_date" value={formData.start_date} onChange={handleChange} className="w-full p-3 border rounded" />
          <input type="datetime-local" name="end_date" value={formData.end_date} onChange={handleChange} className="w-full p-3 border rounded" />
        </div> */}

        {/* Contest Code */}
        {/* <input type="text" name="contest_code" value={formData.contest_code} onChange={handleChange} placeholder="Contest Code (optional)" className="w-full p-3 border rounded" /> */}
        ...
        <div className="space-y-2">
          ...
          {prizeDist.map((pd, idx) => (
            <div key={idx} className="flex gap-2 items-center">
              <input type="number" value={pd.from} onChange={(e) => handlePrizeChange(idx, "from", e.target.value)} placeholder="From" className="w-16 p-2 border rounded" />
              <input type="number" value={pd.to} onChange={(e) => handlePrizeChange(idx, "to", e.target.value)} placeholder="To" className="w-16 p-2 border rounded" />
              <input type="number" value={pd.amount} onChange={(e) => handlePrizeChange(idx, "amount", e.target.value)} placeholder="Amount" className="w-24 p-2 border rounded" />
              {prizeDist.length > 1 && <button type="button" onClick={() => removePrizeRow(idx)} className="px-2 py-1 bg-red-500 text-white rounded">Remove</button>}
            </div>
          ))}
